...
def split_image_dynamic(img):
    h, w, _ = img.shape
    return {
        "header": img[int(h*0.1):int(h*0.4), :],
...
def detect_sender(text):
    tokens = text.split()
    for i, t in enumerate(tokens):
        if t in {"ช่องทางการชำระเงิน", "ช่องทางการชำระเงิน"} and i + 3 < len(tokens):
            return f"{tokens[i+1]}"
    return None
...
def detect_receiver(text):
    tokens = text.split()
    for i, t in enumerate(tokens):
        if t in {"รายละเอียดสินค้า"}:
            return " ".join(tokens[i+1:]).upper()
    return None
...
def truemoney_data(image_bytes: bytes):
